...
def all_search():
    ...
    if 'get_users' in request.form:
        ...
        try:
            ...
            rUsers = session.get('reddit_users', [])
            if not rUsers:
            # If rUsers is not in the session, call get_rUsers
                reddit_dict, rUsers = get_rUsers(keyword)

            yUsers = session.get('youtube_users', [])
            if not yUsers:
            # If rUsers is not in the session, call get_rUsers
                youtube_dict, yUsers = get_yUsers(keyword)

            tUsers = session.get('twitter_users', [])
            if not tUsers:
            # If rUsers is not in the session, call get_rUsers
    ...
    if keyword:
        try:
            ...
            session['reddit_users']=rUsers
            session['youtube_users'] = yUsers
            session['twitter_users'] = tUsers
...
def generate_threads(search_query):
    ...
    results = list(reddit.subreddit('all').search(search_query, time_filter='all', limit=50))
    rthreads_response = []
    try:
...
def get_tUsers(keywords):
    ...
    # Process only the first 2 keywords
